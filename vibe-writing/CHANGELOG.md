...
- `AGENTS.md` 新增「写作风格参考」章节：基于 blognas.hwb0307.com 三篇代表性文章的风格分析，提炼开篇方式、语气人称、段落节奏、类比规范、结构引导、强调手段、禁忌七个维度

## [1.0.1] - 2026-03-22

### Changed（变更）

- 重构 `AGENTS.md`：将通用项目模板替换为面向科普写作的个性化指令
  - 新增「角色定义」章节，明确 AI 技术作家的写作特质
  - 重写「核心工作流」为写作四步骤：理解概念 → 选取类比 → 起草与自检 → 输出规范
  - 将「工程原则」替换为「写作原则」（准确性优先、零专业术语、单一类比等）
  - 删除与写作项目无关的「版本号管理规范」和「Codex CLI 特定说明」章节
  - 精简「有机更新原则」，去除冗余层级
